...
struct IsAssignableFromOptional
    : std::integral_constant<bool, IsConvertibleFromOptional<T, U>::value ||
...
#undef OPTIONAL_DECLSPEC_EMPTY_BASES
  ...
  template <class U>
  CONSTEXPR explicit optional(in_place_t, const U il[], const T& _value)
  ...
  CONSTEXPR T value_or(U&& default_value) const&& {
    ...
    static_assert(std::is_convertible<U, T>::value, "U must be convertible to T");
    return storage_.is_populated_ ? std::move(value())
...
namespace std {
template <class T>
struct hash<AgoraUAP::optional<T> > {
  size_t operator()(const AgoraUAP::optional<T>& opt) const {
    return opt == AgoraUAP::nullopt ? 0 : std::hash<T>()(*opt);
  }
};
}  // namespace std
#endif
